...
public class RenderHandler {
    ...
    public void renderGameOverlay(RenderGameOverlayEvent event) throws IllegalAccessException {
        ...
        try {
            ...
            if (event.getType() == RenderGameOverlayEvent.ElementType.DEBUG) {
                ...
                RayTraceResult mop = mc.getRenderViewEntity().rayTrace(15, 1F);
                    ...
                    if (curBlockDamage != null) {
                        ...
                        if ((!Block.isEqualTo(blockLookingAt, Blocks.air)) && (damage > 0)) {
                            ...
                            if (Setting.blockDamageStyle == 0) {
                                mc.entityRenderer.setupOverlayRendering();
                                String text = String.format("%.1f%%", new Object[]{Float.valueOf(damage * 100)});
                                int color = 0xFFFFFF;
                            ...
                            if (Setting.dropStyle == 0) {
                                ...
                                if (Setting.showDurability) {
                                    ...
                                    if ((damage > 0) && (maxDamage > 0) && (maxDamage - damage > 0)) {
                                        ...
                                        text = String.format("%d/%d", new Object[]{Integer.valueOf(maxDamage - damage), Integer.valueOf(maxDamage)});
                                        fontRender.drawStringWithShadow(text, x, y, color);
                                    }
                                }
                            } else {
                                ...
                                String text = "";
                                if ((damage > 0) && (maxDamage > 0) && (maxDamage - damage > 0)) {
                                    text = String.format(TextFormatting.BOLD + "" + TextFormatting.GOLD + "%s:" + TextFormatting.RESET + " %d/%d", new Object[]{it.getDisplayName(), Integer.valueOf(maxDamage - damage), Integer.valueOf(maxDamage)});
                                } else
                                    text = TextFormatting.BOLD + "" + TextFormatting.GOLD + it.getDisplayName() + TextFormatting.RESET;
                                list.add(I18n.translateToLocal("smartcursor.item.count") + it.stackSize);
                                ...
                                for (int i = 1; i < list.size(); i++)
                                    fontRender.drawStringWithShadow((String) list.get(i), x, y + 8 + fontRender.FONT_HEIGHT * i, color);
    ...
    public String getMobStyleName() {
        ...
        if (Setting.mobStyle == 2) return I18n.translateToLocal("smartcursor.style.icons");
    ...
    public String getPlayerStyleName() {
        ...
        if (Setting.playerStyle == 2) return I18n.translateToLocal("smartcursor.style.progressBar");
    ...
    public void addTooltipText(ItemTooltipEvent event) {
        ...
        event.getToolTip().add(TextFormatting.AQUA + "" + TextFormatting.ITALIC + "Unlocalized name:" + TextFormatting.RESET + stack.getUnlocalizedName());
    }
}